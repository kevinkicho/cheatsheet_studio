...
export function densifyPlacedGroups(
  ...
  for (const [key, members] of groups) {
    ...
    // leaf bbox. Previously we required a strict area shrink — so a shelf
    // layout with the same bounding box never got rewritten to free-flow,
    // and “Auto-layout inside panel” looked magically better (it always
 ...
 * left a sparse shelf inside L2 (e.g. 6.1 Algebra) that only “Auto-layout
 * inside panel” fixed.
 */
